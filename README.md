...
Методы:  
`reset()` - очистка всех полей формы и сообщения об ошибках  
...
<!-- #### buyer:dataChanged
Событие, при котором в модели меняются данные пользователя -->

#### gallery:initialized
Событие, при котором в модели инициализируются карточки с данными о товарах

#### gallery:itemSelected
Событие, при котором пользователь кликнул по карточке товара на главном экране. Влечет создание превью карточки в модальном окне
...
#### orderForm:submit
Событие отправки формы. Влечет сохраниение и валидацию введенных данных в модели покупателя. При отсутствии ошибок валидации выводится окно с формой контактных данных

#### contactsForm:submit
Событие отправки формы. Влечет сохраниение и валидацию введенных данных в модели покупателя. При успешной валидации данные о заказе отправляются на сервер. При успешной отправке выводится окно с информацией о совершенном заказе. При ошибке - сообщение об ошибке создания заказа в UI и информация об ошибке в консоль разработчика
