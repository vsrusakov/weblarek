...
Конструктор:  
`constructor(items: IProduct[], events: IEvents)` - в конструктор передается массив доступных товаров и брокер событий  
...
Конструктор:  
`constructor(events: IEvents)` - принимает экземпляр брокера событий для уведомления об изменении состояния корзины  
...
`clearData(): void` - удаление всех товаров из корзины  
...
`contentElement: HTMLElement` - элемент, содержащий разметку другого компонента для отображения в модальном окне  
`events: IEvents` - брокер событий  

Методы:  
`open()` - метод добавляет окну класс для видимости элемента на странице  
`hide()` - метод удаляет у окна класс для видимости элемента  
`set content(content: HTMLElement)` - сеттер для установки контента внутри окна  
...
`set buttonText(text: TButtonText)` - сеттер для выбора текста кнопки покупки в зависимости от наличия товара в корзине  
...
`set submitDisabled(value: boolean)` - сеттер для установки атрибута disabled у кнопки подтверждения формы  
`reset()` - очистка всех введенных данных формы и сообщения об ошибках  
...
Конструктор:  
`constructor(container: HTMLElement, events: IEvents)` - принимает элемент html, за представление которого будет отвечать класс, и обработчики действий с формой  

Поля:  
`orderButtons: HTMLButtonElement[]` - кнопки для выбора типа оплаты заказа  
`addressInput: HTMLInputElement` - поле ввода адреса доставки  
`events: IEvents` - брокер событий  

Методы:  
`setActivePaymentButton(payment: TPayment)` - метод установки класса `button_alt-active` при клике по кнопкам выбора метода оплаты  
...
Слой презентера реализован в основном скрипте проекта `main.ts`. В нем устанавливаются обработчики событий, которые описаны ниже.
...
#### basket:changed
Событие, при котором изменяется набор товаров в корзине. Влечет перерисовку списка товаров в корзине и счетчика товаров заголовке

#### basket:itemDeleted
Событие, при котором нажимается кнопка удаления товара из корзины. Влечет удаление товара из модели корзины

#### buyer:dataChanged
Событие, при котором в модели меняются данные пользователя. Влечет валидацию данных, вывод ошибок валидации и переход к вывод следующей формы ввода данных покупателя

#### gallery:initialized
Событие, при котором в модели инициализируются карточки с данными о товарах

#### gallery:itemSelected
Событие, при котором в модели каталога товаров устанавливается выбранная карточка. Влечет создание превью карточки в модальном окне

#### galleryCard:selected
Событие, при котором пользователь кликнул по карточке товара на главном экране

#### modal:close
Событие клика для закрытия модального окна

#### previewCard:buy
Событие клика по кнопке добавления товара в корзину или его удаления

#### form:changed
Событие, при котором пользователь вводит данные в форму. Влечет сохранение данных в модели покупателя

#### orderForm:submit
Событие отправки формы. Влечет валидацию введенных данных в модели покупателя. При отсутствии ошибок валидации выводится окно с формой контактных данных

#### contactsForm:submit
Событие отправки формы. Влечет валидацию введенных данных в модели покупателя. При успешной валидации данные о заказе отправляются на сервер. При успешной отправке выводится окно с информацией о совершенном заказе. При ошибке - сообщение об ошибке создания заказа в UI и информация об ошибке в консоль разработчика

#### orderSuccess:close
Событие о закрытии окна с подтверждением заказа
